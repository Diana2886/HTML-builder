  ...
  templateReadStream.on('end', () => {
    ...
    tags.forEach(item => {
      for (let entry of entries) {
        ...
        if (entry.isFile() && path.extname(entry.name) === '.html' && `{{${entry.name.split('.')[0]}}}` === item) {
          let component = '';
...
async function makeBundle(src) {
  ...
  for (let entry of entries) {
    ...
    if (entry.isFile() && path.extname(entry.name) === '.css') {
      ...
      readStream.on('end', () => bundle.forEach(item => writeStream.write(`${item}\n`)));
    }
  }
}
...
copyUpdatedDir(path.join(__dirname, 'assets'), path.join(__dirname, 'project-dist', 'assets'));